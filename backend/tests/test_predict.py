...
# Ensure backend package/module is importable when pytest runs from repo root
...
def patch_proxy_predict(monkeypatch):
    # Replace proxy_predict with a deterministic stub so tests don't need a live proxy or TF model
    ...
    # Ensure MODEL is None so api.py chooses proxy path
...
def test_predict_endpoint_returns_json():
    ...
    # craft a minimal fake image upload (bytes don't need to be valid image for proxy path)
